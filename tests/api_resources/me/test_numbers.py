...
class TestNumbers:
    ...
    def test_streaming_response_list(self, client: Blooio) -> None:
        with client.me.numbers.with_streaming_response.list() as response:
...
class TestAsyncNumbers:
    ...
    async def test_method_list(self, async_client: AsyncBlooio) -> None:
        number = await async_client.me.numbers.list()
        assert_matches_type(NumberListResponse, number, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_list(self, async_client: AsyncBlooio) -> None:
        response = await async_client.me.numbers.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncBlooio) -> None:
        async with async_client.me.numbers.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
